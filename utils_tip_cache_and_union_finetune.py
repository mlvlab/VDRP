...
class DataFactory(Dataset):
    def __init__(self, name, partition, data_root, clip_model_name, zero_shot=False, zs_type='rare_first', num_classes=600, detr_backbone="R50"): ##ViT-B/16, ViT-L/14@336px
        ...
        # Load HICO-DET or V-COCO dataset
        if name == 'hicodet':
        ...
        # Set zero-shot settings for HICO-DET
        ...
        if self.name == 'hicodet' and self.zero_shot and self.partition == 'train2015':
            # Keeping HOI index for training in zero-shot setting
            self.zs_keep = []
            self.remain_hoi_idx = [i for i in np.arange(600) if i not in self.filtered_hoi_idx]

            # Filter out data samples for zero-shot setting
            for i in self.keep:
            ...
            # Filter out object classes for zero-shot setting
    ...
    # Get the image and target for the given index (self.keep[i])
    def __getitem__(self, i):
        ...
        image_orig, _ = self.clip_transforms(image, target)
        image, target = self.transforms(image, target)
        image_clip, target = self.clip_transforms(image, target)  
        
        # Normalize the image and target
        image, _ = self.normalize(image, None)
        image_clip, target = self.clip_normalize(image_clip, target)
        image_orig, _ = self.clip_normalize(image_orig, None)

        target['filename'] = filename
        
        return (image,image_clip,image_orig), target
    
    # Get the square images using background color
...
class CustomisedDLE(DistributedLearningEngine):
    ...
    def _on_each_iteration(self):
        ...
        self._state.loss.backward(retain_graph=True)
        if self.max_norm > 0:
